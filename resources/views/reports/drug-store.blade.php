...
@section('title', __('report.drug_store_report'))

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('reports.index') }}">{{ __('report.title') }}</a></li>
    <li class="breadcrumb-item active">{{ __('report.drug_store_report') }}</li>
@endsection
...
<div class="mb-3 d-flex align-items-center gap-2">
    ...
    <h5 class="fw-bold mb-0" style="color:#1e293b;">
        <i class="bi bi-box-seam-fill me-2" style="color:#d97706;"></i>{{ __('report.drug_store_report') }}
    </h5>
</div>
    ...
    <div class="card-body">
        <form method="GET" action="{{ route('reports.drug-store') }}" class="row g-3 align-items-end">
            <div class="col-sm-5 col-md-4">
                <label class="form-label fw-semibold" style="font-size:.85rem;">{{ __('report.filter_type') }}</label>
                <select name="drug_type_id" class="form-select">
                    <option value="">{{ __('report.all_types') }}</option>
    ...
    <div class="col-6 col-md-3">
        <div class="card text-center h-100">
            <div class="card-body py-3">
                ...
                <div class="text-muted" style="font-size:.78rem;">{{ __('report.total_items') }}</div>
            </div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="card text-center h-100">
            <div class="card-body py-3">
                ...
                <div class="text-muted" style="font-size:.78rem;">{{ __('report.total_stock_units') }}</div>
            </div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="card text-center h-100">
            <div class="card-body py-3">
                ...
                <div class="text-muted" style="font-size:.78rem;">{{ __('report.low_stock') }}</div>
            </div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="card text-center h-100">
            <div class="card-body py-3">
                ...
                <div class="text-muted" style="font-size:.78rem;">{{ __('drug.out_of_stock') }}</div>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="card-body p-0">
        @if($drugs->isEmpty())
            <div class="text-center text-muted py-5">
                ...
                <div class="mt-2">{{ __('drug.no_drugs') }}</div>
            </div>
        @else
        <div class="table-responsive">
            <table class="table table-hover mb-0" style="font-size:.84rem;">
                <thead style="background:#f8fafc;">
                    <tr>
                        <th class="ps-3">#</th>
                        <th>{{ __('report.drug_name') }}</th>
                        <th>{{ __('report.type_category') }}</th>
                        <th>{{ __('field.unit') }}</th>
                        <th class="text-end pe-3">{{ __('drug.stock_qty') }}</th>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            @if($drug->stock_quantity === 0)
                                <span class="badge bg-danger">{{ __('drug.out_of_stock') }}</span>
                            @elseif($drug->isLowStock())
                                <span class="badge" style="background:#d97706;">{{ __('report.low_stock') }}</span>
                            @else
                                <span class="badge" style="background:#10b981;">{{ __('report.in_stock') }}</span>
                            @endif
                        </td>
                    </tr>
                    @endforeach
